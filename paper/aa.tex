...
%-----------------------------------------------------------------
\abstract
{Publicly available NBA box-score and standings data make it possible to build detailed models of team and player performance, yet many forecasting systems still rely on simple team-level summaries such as past wins or win percentage. In this project, we systematically compare team-centric and player-centric pipelines for predicting regular-season team performance and related player outcomes over roughly two decades of NBA data. First, we build a historical-standings baseline that regresses team win percentage on features constructed from past seasons (previous win rate, rolling 3-year and 5-year averages, and short-term trends). Second, we train player-level models on season-long box-score statistics to: (i) predict All-Star selection, (ii) predict whether a player receives All-NBA/MVP–related honors, and (iii) classify players into broad positional groups (Guard / Wing / Big). Third, we aggregate player statistics back to the team level to construct player-informed team features and use these to predict next-season team win percentage. Finally, we implement an automatic model search over feature groups, preprocessing schemes, and regression algorithms, selecting configurations that maximize ranking-based metrics on the two most recent seasons. Across held-out seasons, historical win–trend models already achieve high $R^2$ and correlation with true win percentage, while player-aggregated models and the auto-optimized pipeline further improve ranking accuracy for recent seasons. Our results suggest that simple team-level trends are strong baselines, but player-level modeling and automated pipeline search add value for capturing league-wide standings and connecting roster composition to expected team performance.}
...
A natural starting point is a team-centric view: treat each franchise as a single unit and summarize its history through wins, losses, and a small set of trend features. This mirrors many real-world forecasting systems that rely on past win percentage and simple trends to project future success. Such models are easy to interpret and compute, but they ignore who is actually on the roster and how individual players evolve over time.

Complementing this is a player-centric view, where we model players directly and then reason upward to teams. Season-level player box-score statistics capture scoring, playmaking, defense, and usage patterns that drive both individual recognition and team success. In this project, we use player data to tackle three prediction problems: (1) identifying All-Star–caliber seasons, (2) predicting whether a player will receive All-NBA or MVP–related honors, and (3) classifying players into broad positional groups (Guard / Wing / Big). These tasks test whether basic box-score data is sufficient to recover meaningful structure in how the league evaluates and uses players.
...
Together, these components allow us to ask a concrete question: when is a simple team-level win–trend baseline “good enough,” and when do richer player-level features and automated model selection provide real gains in predicting how the NBA standings will shake out? This project aims to quantify that trade-off and clarify how much value is added by moving from coarse team summaries to more detailed, player-informed representations.
...
Although our project does not have deployed “end users,” we still treat
evaluation as a user-centered process. The intended users are basketball
fans and analysts who care about both accuracy and interpretability in
...
approximate realistic forecasting. Rather than randomly splitting within
a season, we train on earlier seasons and evaluate on the most recent:

\begin{itemize}
  \item \textbf{Team-level baselines (Tasks 1, 3, 4):}
  we train on all seasons up to 2022--2023 and treat 2023--2024 and
  2024--2025 as held-out test seasons. Feature engineering (e.g., rolling
  averages) uses only past seasons for each team.
  \item \textbf{Player-level classification (Task 2):}
  for All-Star and MVP/All-NBA prediction we train on earlier seasons
  and hold out the last three. For position prediction we train on
  seasons up to 2022--2023 and test from 2023--2024 onward.
\end{itemize}

\subsection{Metrics by task and iteration}

We use different metrics for team-level and player-level tasks, as
outlined in Section~\ref{sec:notation}, and we track how these metrics
change as we iterate on features and models.

For the standings-only baseline and the player-to-team aggregation
model, we monitor MAE, RMSE, $R^2$, and correlations on the two most
recent seasons. Once we observed that a small set of trend features
already achieved low MAE and high $R^2$, we fixed this trend-based
baseline as a strong reference point.

In the auto-tuned team model, we focus on ranking metrics (ExactAcc,
Within1, Within2). We define a combined score
that emphasizes Within1 and use the average of these metrics across the
two held-out seasons as the optimizer’s objective.

For the All-Star and MVP/All-NBA tasks, early prototypes that optimized
only accuracy performed poorly on the rare positive class. Inspecting
classification reports and confusion matrices revealed that the model
often predicted “no award” for almost everyone. In response, we
introduced class weighting in the MVP model, monitored positive-class
recall and F1 as primary metrics, and adjusted thresholds to balance
precision and recall.

For the position classifier, comparing against the “always Guard”
baseline and inspecting confusion matrices showed that Guards and Bigs
were learned relatively well, while Wings were harder to distinguish.
This guided feature engineering toward adding per-36 rebounding,
shooting volume, and usage-related features to better separate wings
from guard-sized and big-sized players.

For the player-to-team aggregation pipeline, we compared MAE, RMSE, and
correlations to the standings-only baseline on the same held-out
seasons. High-dimensional variants did not clearly improve these metrics
and were difficult to interpret, so we kept the simpler aggregation that
sums player stats and recomputes standard team-level efficiencies.

\subsection{Informal user feedback}

We gathered informal feedback from classmates who follow the NBA by
showing:

\begin{itemize}
  \item side-by-side tables of predicted vs.\ actual rankings,
  \item scatter plots of predicted vs.\ actual rankings, and
  \item examples of All-Star and MVP predictions with underlying
        box-score features.
\end{itemize}

They found the “within one rank” metric and ranking scatter plots more
intuitive than raw error tables, and preferred models whose mistakes
were small shifts in rank rather than large misorderings. This feedback
reinforced our choice to emphasize ranking metrics in the team-level
models and to present classification performance with per-class
precision and recall, not just overall accuracy.
...
Much of next season’s variance can be explained by
...
type of model already provides a solid answer.
...
playmaking, and long-term trends to sort teams more precisely.
...
beyond simple standings is most useful when grounded in interpretable
basketball quantities rather than opaque feature engineering.

\subsection{What player-level models add}

The player-level tasks highlight a different dimension.
...
Aggregating player statistics back to the team level yields performance
comparable to the standings baseline and clarifies where that baseline
may break: when a roster changes dramatically or when a team’s star
power collapses or spikes.
...
\begin{itemize}
    ...
    \item The feature-rich team model offers a measurable boost in
          ranking accuracy while still using familiar statistics.
